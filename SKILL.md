...
7. 每条主路径必须有明确出口，别让客户困在某个节点里转圈。**提示词里写"进入售后/进入某流程"
   不等于有出口——每个流程出口都必须有对应的分支或连线**，否则客户到这一步只能在本节点循环。
8. **systemPrompt 的「相关信息」只列外部传入变量**（`环境注入变量` 里声明的、通话开始就有值的）。
   词槽采集 / 接口返回 / 变量赋值产生的内部变量一律不写进全局提示词——那时它们还没有值，
   写进去会诱导大模型提前编造。内部变量在产生它之后的节点话术里交代（校验报 W10）。
